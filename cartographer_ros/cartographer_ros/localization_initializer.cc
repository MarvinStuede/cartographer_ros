...
void HandleInitialPose(
  ...
  if (pose_msg.header.frame_id == "map") {
    ...
    ros::ServiceClient client_trajectory_states =
    ...
    cartographer_ros_msgs::GetTrajectoryStates srv_trajectory_states;
    if(!client_trajectory_states.call(srv_trajectory_states)){
      LOG(ERROR) << "Error getting trajectory states";
      return;
    }
    //Finish active trajectory
    if(srv_trajectory_states.response.trajectory_states.trajectory_state.back() ==
       srv_trajectory_states.response.trajectory_states.ACTIVE){
      int trajectory_id = srv_trajectory_states.response.trajectory_states.trajectory_id.back();
      LOG(INFO)<<"Finishing trajectory "<<trajectory_id;
      srv_finish.request.trajectory_id = trajectory_id;
      if(!client_finish.call(srv_finish)) {
        LOG(ERROR) << "Error finishing trajectory";
        return;
      }
    }
    //Finish all active trajectories
//    for(std::size_t i=0; i < srv_trajectory_states.response.trajectory_states.trajectory_id.size();i++){
//      if(srv_trajectory_states.response.trajectory_states.trajectory_state[i] ==
//         srv_trajectory_states.response.trajectory_states.ACTIVE){
//        int trajectory_id = srv_trajectory_states.response.trajectory_states.trajectory_id[i];
//        LOG(INFO)<<"Finishing trajectory "<<trajectory_id;
//        srv_finish.request.trajectory_id = trajectory_id;
//        if(!client_finish.call(srv_finish)) {
//          LOG(ERROR) << "Error finishing trajectory";
...
//    }
    ...
    srv_start.request.topics.nav_sat_fix_topic = nh->resolveName(kNavSatFixTopic,true);
    srv_start.request.topics.multi_echo_laser_scan_topic = nh->resolveName(kMultiEchoLaserScanTopic,true);
